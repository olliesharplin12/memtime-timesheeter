...
# TODO: How to type tuples correctly?
...
def filter_tasks_to_create(upcoming_tasks: List[dict], memtime_tasks: List[Task]) -> List[(dict, str)]:
...
def map_tasks_to_memtime_project(tasks_to_create: List[dict], memtime_projects: List[Project]) -> List[(dict, str, Project)]:
    valid_tasks_to_create: List[(dict, str, Project)] = []
...
def confirm_and_create_projects(projects_to_create: List[(int, str)]):
    print('\nNew projects:')
...
def confirm_and_create_tasks(tasks_to_create: List[(dict, str, Project)]):
    print('\nNew tasks:')
    for task, _, _ in tasks_to_create:
